...
4. Base branch resolved: default branch from `gh repo view --json defaultBranchRef` (fallback `git symbolic-ref refs/remotes/origin/HEAD`).
...
3. **Create the branch.** `git checkout -b <branch> <base>`. If the branch exists, check it out and continue.
