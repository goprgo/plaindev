...
- Prefer positive phrasing. Avoid stacked negatives.
- Define jargon on first use, in parentheses.
- One term per concept. Reuse it throughout. Do not swap synonyms to avoid sounding repetitive. Repetition aids clarity.
...
- Bold the core concept in a sentence to guide the eye. At most one bold phrase per paragraph.
- Long answers (escape hatch) start with a one-line TL;DR.
- Show `// Before` and `// After` in code blocks for diffs.

## Response shapes

Pick the shape that matches the request. Use the same shape for the same kind of task, every time.

`Next step` is optional. Include it only when the user must act. Drop it for trivia.
...
> "The handler catches the error. The exception is logged. The failure is returned to the client."
